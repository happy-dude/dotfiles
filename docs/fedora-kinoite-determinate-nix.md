...
This runbook covers the reusable installation and application configuration. Two
companion documents hold the material that would otherwise churn it:

- [Upgrade and rollback](fedora-kinoite-upgrade-rollback.md) — rollback and
  recovery layers, retired Toolbx state, and the major Fedora upgrade checklist.
- [Worldmind migration history](fedora-kinoite-migration-history.md) — the dated
  `worldmind` native deployment record referenced above.
