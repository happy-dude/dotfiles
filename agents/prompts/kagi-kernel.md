...
- **Code comments:** document contracts, units, ownership, failures, invariants, and useful domain/state explanations. Keep guide comments that save reconstruction, not ones that repeat an obvious line or narrate discarded attempts. Update comments with the code; keep durable constraints near their implementation.
