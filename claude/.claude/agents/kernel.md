...
- **Layout:** `flake.nix` declares one Home Manager output (`homeConfigurations."schan"`, `x86_64-linux`, generic Linux + nixGL — not NixOS). `home.nix` is the entry module and holds the flat `home.packages` list; per-app config lives in its own `<app>/default.nix`, imported via `flake.nix`'s `modules` list (`aerc/`, `emacs/`, `fish/`, `fonts/`, `ghostty/`, `nix/`, `xdg/`, `tldr/`, `tmux/`, `wezterm/`, `zsh/`, `yt-dlp/`). A handful of top-level dirs (`bat/`, `editorconfig/`, `gdb/`, `git/`, `go/`, `karabiner/`, `org-dirs/`, `rime/`, `roswell/`, `ssh/`, `terminfo/`, plus `zed/` and `claude/`) are GNU Stow packages instead — used when a config must stay writable in place (an app rewrites its own settings file, or a prompt/dotfile is meant to be hand-edited) rather than symlinked read-only into the Nix store.
...
- **To actually add or change something persistently on this machine:** edit `home.nix` (or the relevant `default.nix`), run `nix fmt .` (formats via `nixfmt-tree`, repo convention after any `.nix` edit), then `home-manager switch --flake .#schan --show-trace` to apply — this changes the live environment, so confirm with the user first like any other install. `./scripts/update.sh` is the one-shot full-sync orchestrator (Rime → git → submodules → nvim → `nix fmt` → `home-manager switch`, with per-step `--skip-*` flags) if asked to "update everything."
