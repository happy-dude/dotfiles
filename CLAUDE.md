...
- **`rime/`** is a Home Manager module (`rime/default.nix`) over a retained Stow-compatible snapshot. Locked Git schema inputs replace matching snapshot files in Home Manager mode; `pkgs.rime-zhwiki` supplies the generated Zhwiki dictionary. Activation materializes immutable schema data under `~/.local/share/fcitx5/rime/.home-manager-static` and links the active files there, so host Fcitx can use a configuration built inside Toolbox without accessing its `/nix/store`. Fcitx configuration and themes link directly to the shared dotfiles checkout. Local schema overrides and Lua files remain in the snapshot, while Rime user data stays writable for generated build output and learned state. When managed sources change, activation invalidates only the generated build cache and reloads Rime; if Fcitx is not running, it rebuilds on its next start.
...
Default Rime updates happen through `nix flake update`; `--rime-source nix` is implicit. The following Home Manager activation rebuilds generated schemas when the static Rime source stamp changes, so no manual deploy is required. `--rime-source plum` runs the legacy installer only with `--skip-home-manager` and refuses if the current Rime files resolve into the Nix store. Add `--skip-nix-flake` for a pure Stow/Plum update.
