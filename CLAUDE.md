...
- `flake.nix` declares per-user Home Manager outputs through `mkHome { username, desktop, rimeDeployment }`: `homeConfigurations."schan"` is the personal Fedora Kinoite (KDE Plasma) profile at `/home/schan`, and `homeConfigurations."stachan"` is the work GNOME profile at `/home/stachan`. Both are `x86_64-linux` Home Manager configurations for generic Linux rather than NixOS. `username` determines `homeDirectory`; `desktop` selects session integration; and `rimeDeployment` selects `nix` (the default) or `stow` file management. Each machine switches its own output (`.#schan` / `.#stachan`); `scripts/update.sh` defaults to `.#$(whoami)`.
- On the personal Kinoite system, Nix and Home Manager run inside Toolbox, while desktop processes run on the host. Host processes cannot follow Toolbox-only `/nix/store` paths, so host-facing integrations use materialized files in the shared home directory or links into the dotfiles checkout. This constraint explains the Rime and desktop-integration workarounds below.
...
- Source-only inputs also lock Prezto (with submodules), the active Fish plugins, Roswell, RustOwl, and bgutil-ytdlp-pot-provider. `nix flake update` advances those sources together with the Rime schema inputs.
...
- **`rime/`** is a Home Manager module (`rime/default.nix`) over a retained Stow-compatible snapshot. Locked Git schema inputs replace matching snapshot files in Home Manager mode; `pkgs.rime-zhwiki` supplies the generated Zhwiki dictionary. Nix mode claims ownership with `~/.local/state/rime/home-manager-ownership-v1` before changing managed paths, rejects malformed or unmanaged conflicts, materializes locked schema data under `~/.local/share/fcitx5/rime/.home-manager-static`, and links the active files there so host Fcitx can use a Toolbox-built configuration without accessing `/nix/store`. Stow mode validates the ownership marker and every link it will release before deleting anything, then releases only Home-Manager-owned paths; unrelated and generated user data remain intact. When managed sources change, activation invalidates only the generated build cache and reloads Rime; if Fcitx is not running, it rebuilds on its next start.
...
- `xdg/default.nix` performs a custom `home.activation.createHostConfig` step that copies Ghostty `.desktop` files and icons into `$HOME/.local/share` so they're visible to the host system when running inside Toolbox. Keep this in mind when changing Ghostty-related desktop integration.
...
- The README's GNU Stow instructions and `scripts/install.sh` are kept for historical reference as the old whole-repo bootstrap flow — don't extend that flow. New per-app config still defaults to a Home Manager module. For files consumed inside the Nix environment, `config.lib.file.mkOutOfStoreSymlink` can keep a repo source live-editable. Host processes cannot follow Toolbox-only `/nix/store` paths; use activation-created links into `$HOME/dotfiles` or copied host-visible files to cross that boundary, as Rime and Ghostty desktop assets do.
- **`rime/`** creates direct host-visible links for Fcitx profile/config/theme files into `~/dotfiles/rime`, then materializes locked Git schema inputs, the packaged Zhwiki dictionary, and local overrides under `~/.local/share/fcitx5/rime/.home-manager-static`. Its activation links active static files to that snapshot so host Fcitx can read a Toolbox-built deployment. It compares a stamp of the local overrides, schema inputs, and Zhwiki package; on a change it refreshes only that static snapshot, clears generated `build/` data, and reloads the Rime addon. The `stow` mode removes only Home Manager-owned links so the retained snapshot can be deployed with Stow; its local ignore file excludes Home Manager modules. Keep generated state out of Git; `rime/.gitignore` covers the installation metadata and build directory.
