...
- **`rime/`** links locked Git schema inputs, the packaged Zhwiki dictionary, and local overrides individually when `rimeDeployment = "nix"` so Rime can write generated state under `~/.local/share/fcitx5/rime`. Its activation compares a stamp of the local overrides, schema inputs, and Zhwiki package; on a change it clears only generated `build/` data and reloads the Rime addon. The `stow` mode releases those paths to the retained snapshot, whose local ignore file excludes Home Manager modules. Keep generated state out of Git; `rime/.gitignore` covers the installation metadata and build directory.
