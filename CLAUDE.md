...
`bat/`, `git/`, `zed/`, `claude/`, and `rime/` are Home-Manager-managed, not Stow packages — do not `stow` them.
...
- **`rime/`** is a Home Manager module (`rime/default.nix`). It links static Fcitx configuration, themes, schemas, Lua files, and OpenCC data, but leaves the containing Rime user-data directory writable for generated build output and learned state. Do not `stow` it or add runtime state to Git.
...
- **`rime/`** links static files individually through Home Manager so Rime can write generated state under `~/.local/share/fcitx5/rime`. Keep generated state out of Git; `rime/.gitignore` covers the installation metadata and build directory.
