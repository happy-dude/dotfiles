...
- **`rime/`** materializes locked Git schema inputs, the packaged Zhwiki dictionary, and local overrides under `~/.local/share/fcitx5/rime/.home-manager-static` when `rimeDeployment = "nix"`, then links active static files there so host Fcitx can read a Toolbox-built deployment. Its activation compares a stamp of the local overrides, schema inputs, and Zhwiki package; on a change it refreshes only that static snapshot, clears generated `build/` data, and reloads the Rime addon. The `stow` mode releases those paths to the retained snapshot, whose local ignore file excludes Home Manager modules. Keep generated state out of Git; `rime/.gitignore` covers the installation metadata and build directory.
