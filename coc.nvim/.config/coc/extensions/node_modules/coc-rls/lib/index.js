...
Object.defineProperty(exports, "__esModule", { value: true });
const child_process = __importStar(__webpack_require__(1));
...
const rustup_1 = __webpack_require__(33);
const spinner_1 = __webpack_require__(35);
const child_process_1 = __webpack_require__(34);
const signatureHelpProvider_1 = __importDefault(__webpack_require__(36));
let client;
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        let { subscriptions } = context;
        let workspaceFolder = coc_nvim_1.workspace.workspaceFolders.find(workspaceFolder => {
            let folder = coc_nvim_1.Uri.parse(workspaceFolder.uri).fsPath;
            return fs.existsSync(path_1.default.join(folder, 'Cargo.toml'));
        });
        let channel = coc_nvim_1.workspace.createOutputChannel('rls');
        if (!workspaceFolder) {
            channel.appendLine(`[Warning]: A Cargo.toml file must be at the root of the workspace in order to support all features`);
        }
        let folder = workspaceFolder ? coc_nvim_1.Uri.parse(workspaceFolder.uri).fsPath : coc_nvim_1.workspace.rootPath;
        client = new ClientWorkspace({
            uri: coc_nvim_1.Uri.file(folder).toString(),
            name: path_1.default.basename(folder),
        }, channel);
        client.start(context).catch(_e => {
            // noop
        });
        subscriptions.push(coc_nvim_1.workspace.onDidChangeWorkspaceFolders(e => {
            if (e.added) {
                ...
                if (folder)
                    channel.appendLine(`[Warning]: Multiple rust workspace folder not supported!`);
            }
        }));
    });
}
...
class ClientWorkspace {
    constructor(folder, channel) {
        this.channel = channel;
        this.lc = null;
        this.config = configuration_1.RLSConfiguration.loadFromWorkspace(coc_nvim_1.Uri.parse(folder.uri).fsPath);
        this.folder = folder;
    }
    start(context) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.lc = new coc_nvim_1.LanguageClient('rust-client', 'Rust Language Server', serverOptions, clientOptions);
            context.subscriptions.push(coc_nvim_1.languages.registerSignatureHelpProvider(['rust'], new signatureHelpProvider_1.default(this.lc), ['(', ',',]));
    ...
    registerCommands(context) {
        ...
        const restartServer = coc_nvim_1.commands.registerCommand('rls.restart', () => __awaiter(this, void 0, void 0, function* () {
            if (this.lc) {
                this.lc.stop();
            }
        ...
        let terminal;
        context.subscriptions.push(coc_nvim_1.commands.registerCommand('rls.run', () => __awaiter(this, void 0, void 0, function* () {
            if (terminal)
                terminal.dispose();
            terminal = yield coc_nvim_1.workspace.createTerminal({
                name: 'cargo run',
                cwd: coc_nvim_1.workspace.rootPath,
                env: process.env
            });
            terminal.show(true);
    ...
    progressCounter() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.lc.onNotification(new vscode_languageserver_protocol_1.NotificationType('window/progress'), (progress) => {
                if (progress.done) {
    ...
    makeRlsEnv(setLibPath = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                env.PATH = `${os_1.default.homedir()}/.cargo/bin:${env.PATH || ''}`;
                try {
                    sysroot = yield this.getSysroot(env);
                }
                catch (e) {
                    ...
                    coc_nvim_1.workspace.showMessage(`Error reading sysroot: ${e.message}`, 'error');
                    return env;
                }
            }
            ...
            if (setLibPath) {
                ...
                env.DYLD_LIBRARY_PATH = appendEnv('DYLD_LIBRARY_PATH', path_1.default.join(sysroot, 'lib'));
                env.LD_LIBRARY_PATH = appendEnv('LD_LIBRARY_PATH', path_1.default.join(sysroot, 'lib'));
            }
            return env;
        });
    }
    makeRlsProcess() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let childProcess;
            if (rls_path) {
                const env = yield this.makeRlsEnv(this.config.setLibPath);
                this.channel.appendLine(`running: ${rls_path} at ${coc_nvim_1.workspace.rootPath}`);
                childProcess = child_process.spawn(rls_path, [], { env, cwd: coc_nvim_1.workspace.rootPath });
            }
            else if (this.config.rustupDisabled) {
                const env = yield this.makeRlsEnv(this.config.setLibPath);
                this.channel.appendLine(`running: rls at ${coc_nvim_1.workspace.rootPath}`);
                childProcess = child_process.spawn('rls', [], { env, cwd: coc_nvim_1.workspace.rootPath });
            }
            else {
                ...
                this.channel.appendLine(`running: ${config.path} run ${config.channel} rls, at ${coc_nvim_1.workspace.rootPath}`);
                childProcess = child_process.spawn(config.path, ['run', config.channel, 'rls'], { env, cwd: coc_nvim_1.workspace.rootPath });
            }
            childProcess.on('error', (err) => {
                if (err.code === 'ENOENT') {
                    spinner_1.stopSpinner('RLS could not be started');
                    coc_nvim_1.workspace.showMessage(`Could not spawn RLS: ${err.message}`, 'error');
                    this.channel.appendLine(`Could not spawn RLS: ${err.message}`);
                }
            });
            if (this.config.logToFile) {
                const logPath = path_1.default.join(coc_nvim_1.workspace.rootPath, 'rls' + Date.now() + '.log');
...
const sr = __webpack_require__(31);
var Proposed;
(function (Proposed) {
...
const messageReader_1 = __webpack_require__(10);
exports.MessageReader = messageReader_1.MessageReader;
exports.StreamMessageReader = messageReader_1.StreamMessageReader;
exports.IPCMessageReader = messageReader_1.IPCMessageReader;
exports.SocketMessageReader = messageReader_1.SocketMessageReader;
const messageWriter_1 = __webpack_require__(12);
...
const cancellation_1 = __webpack_require__(13);
...
Object.defineProperty(exports, "__esModule", { value: true });
const vscode_jsonrpc_1 = __webpack_require__(7);
const vscode_languageserver_types_1 = __webpack_require__(19);
/**
...
class RLSConfiguration {
    ...
    get setLibPath() {
        return this.configuration.get('rust-client.setLibPath', true);
    }
    get rustupDisabled() {
        const rlsOverriden = Boolean(this.rlsPath);
        return rlsOverriden || this.configuration.get('rust-client.disableRustup', false);
    }
...
function tryToInstallToolchain(config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            let res = yield coc_nvim_1.workspace.runTerminalCommand(config.path + ' toolchain install ' + config.channel, coc_nvim_1.workspace.rootPath);
...
function installRls(config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let install = (component) => __awaiter(this, void 0, void 0, function* () {
            let cmd = config.path + ` component add ${component} --toolchain ` + config.channel;
            let res = yield coc_nvim_1.workspace.runTerminalCommand(cmd, coc_nvim_1.workspace.cwd, true);
            if (!res.success) {
        ...
        try {
            ...
            const hasRls = yield hasRlsComponents(config);
            if (!hasRls)
                throw new Error(`${REQUIRED_COMPONENTS.join(',')} not exists in ${config.channel}`);
        }
        catch (e) {
            spinner_1.stopSpinner('components install failed');
            coc_nvim_1.workspace.showMessage(e.message);
            throw e;
        }
...
class Provider {
    ...
    provideSignatureHelp(document, position, token) {
        ...
        let triggerCharacter = document.getText(vscode_languageserver_protocol_1.Range.create({ line, character: character - 1 }, position));
        ...
        if (triggerCharacter === '(') {
            ...
            return this.provideHover(this.languageClient, document, position, token).then(hover => this.hoverToSignatureHelp(hover));
        }
        else if (triggerCharacter === ',') {
            if (this.previousFunctionPosition &&
                position.line === this.previousFunctionPosition.line) {
                return this.provideHover(this.languageClient, document, this.previousFunctionPosition, token).then(hover => this.hoverToSignatureHelp(hover));
            }
            else {
                return null;
            }
    ...
    provideHover(lc, document, position, token) {
        return new Promise((resolve, reject) => {
            lc.sendRequest(vscode_languageserver_protocol_1.HoverRequest.type, {
                textDocument: {
                    uri: document.uri,
                },
                position: { line: position.line, character: position.character - 1 }
            }, token).then(data => resolve(data), error => reject(error));
        });
    }
    hoverToSignatureHelp(hover) {
        /*
        ...
        */
        ...
        const doc = texts.length > 1
            ? (texts.slice(-1)[0])
            : undefined;
        let si = vscode_languageserver_protocol_1.SignatureInformation.create(label, doc);
...
function getHoverTexts(hover) {
    ...
    if (Array.isArray(contents)) {
        res = contents.map(o => {
            return typeof o.value == 'string' ? o.value : o.toString();
        });
    }
